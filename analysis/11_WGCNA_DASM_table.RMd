...
combo <- dasm %>%
  ...
  dplyr::rename( `Expression Change in Untreated vs. Control` = DEG_Untreated ) %>%
  dplyr::rename( `Expression Change with Treatment` = TreatmentEffect ) %>%
  dplyr::rename( `DASM Correlation` = DASM.Correlation ) %>%
  dplyr::rename( `DASM pvalue` = DASM.pvalue ) %>%
  dplyr::rename( `DAST Correlation` = DAST.Correlation ) %>%
  dplyr::rename( `DAST pvalue` = DAST.pvalue ) %>%
  select( gene_id, symbol, `Expression Change in Untreated vs. Control`, `Expression Change with Treatment`, `DASM Correlation`, `DASM pvalue`, `DAST Correlation`, `DAST pvalue` ) %>%
  arrange( `DASM pvalue`, `DAST pvalue` ) %>%
  write_csv( x = ., path = here::here( 'results', 'DASM_y_DAST_table.csv' ) )

combo = combo %>%
  mutate( SignifDASM = case_when( is.na( `DASM pvalue` ) ~ "Is_NA", `DASM pvalue` < 0.05 ~ "Yes", TRUE ~ "No" ) ) %>%
  mutate( SignifDAST = case_when( is.na( `DAST pvalue` ) ~ "Is_NA", `DAST pvalue` < 0.05 ~ "Yes", TRUE ~ "No" ) ) %>%
  mutate( CorrTypeDASM = case_when( is.na( `DASM Correlation` ) ~ "Is_NA", `DASM Correlation` < 0.0 ~ "Negative", TRUE ~ "Positive" ) ) %>%
  mutate( CorrTypeDAST = case_when( is.na( `DAST Correlation` ) ~ "Is_NA", `DAST Correlation` < 0.0 ~ "Negative", TRUE ~ "Positive" ) )
...
in_list <- dir( wgcna_output_path, pattern = "^GephiInput-edges.+.txt$" )
...
for( idx in 1:length( in_list ) ) {
  ...
  input %>%
    ...
    mutate( .data = ., Type = "Undirected" ) %>%
    select( .data = ., Source, Target, weight, Type ) %>%
