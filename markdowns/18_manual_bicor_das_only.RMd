...
no_pseudogene_ids <- ensembl %>%
  mutate( keepers = case_when(
    str_detect( string = gene_biotype, pattern = "seudo" ) ~ "No",
    TRUE ~ "Yes"
  ) ) %>%
  filter( keepers == "Yes" ) %>%
  pull( gene_id )
...
temp_expression <- read_tsv(
  ...
  filter( gene_id %in% no_pseudogene_ids )
...
melted_bicor <- temp %>%
  ...
  write_csv( x = ., path = here( 'results', 'bicor_network_edges_dasonly_nopseudo.csv' ) )
...
union( x = melted_bicor$Source, y = melted_bicor$Target ) %>%
  ...
  write_csv( x = ., path = here( 'results', 'bicor_network_nodes_dasonly_nopseudo.csv' ) )
```
